  ...
  void initState() {
    ...
    Future.delayed(Duration(seconds: 10), () {
      Navigator.pushReplacement(
        ...
        MaterialPageRoute(
          builder: (context) =>
              LoginScreen(), // Replace HomeScreen with your desired screen
        ),
      );
    });
  }
  ...
  Future<void> paint(Canvas canvas, Size size) async {
    ...
    for (var i = 0; i < noOfImages; i++) {
      ...
      canvas.drawImage(
          image[i],
          pointOnArc +
              Offset(-(image[i].width / 2).toDouble(),
                  -(image[i].height / 2).toDouble()),
          Paint()..colorFilter = filter);
      canvas.drawArc(
          rect,
          degreesToRads((i * arcAngle) + image[i].width / 2)
              .toDouble(), //startAngle
